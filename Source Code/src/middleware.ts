...
export async function middleware(request: NextRequest) {
  ...
  const isAnalyticsPath = url.pathname.startsWith('/user-dashboard/analytics')
  ...
  if (token) {
    ...
    if (isAnalyticsPath && token.role !== 'admin') {
      return NextResponse.redirect(new URL('/user-dashboard/overview', request.url))
    }
